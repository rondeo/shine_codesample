...
module DeviseTokenAuth::Concerns::SetUserByToken
  ...
  def set_user_by_token(mapping = nil)
    ...
    registering_name = 'is-registering'

    # parse header for values necessary for authentication
    uid = request.headers[uid_name] || params[uid_name]
    @token     ||= request.headers[access_token_name] || params[access_token_name]
    @client_id ||= request.headers[client_name] || params[client_name]
    @is_registering ||= request.headers[registering_name]
    puts '----------------------'
    puts "UID #{uid}"
    puts "Client #{@client_id}"
    puts "Token: " + @token.inspect
    puts "Is registering? #{@is_registering}"
    puts '--------------------'
    ...
    puts user && user.valid_token?(@token, @client_id).inspect
    if (
        user &&
        # HACK: allow is_registering header to allow non-approved user to authenticate a request
        # - used to set user avatar in tutor application process before user is approved
        (user.approved? || @is_registering) &&
